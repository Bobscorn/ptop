...
EXECUTION_STATUS process_server_data(client_init_kit& kit, const Message& mess, std::string port, std::unique_ptr<IDataSocket>& conn_socket, int& auth_key_out, const protocol& proto)
{
    try
    {
        const char* data = mess.Data.data();
...
EXECUTION_STATUS connect_peer(const char* data, int& auth_key_out, int i, MESSAGE_LENGTH_T data_len, std::unique_ptr<IDataSocket>& conn_socket) {
    ...
    try
    {
        ...
        while (init.status == EXECUTION_STATUS::CONTINUE) //listen at the start of protocol
        {
            ...
            if (init.conn_socket->has_message())
            {
                ...
                init.status = process_server_data(init, data, Sockets::ClientListenPort, init.conn_socket, init.auth_key, input_protocol);
            }
