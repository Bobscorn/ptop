...
peer_data ILinuxSocket::get_peer_data()
{
	...
	if (!str)
		throw runtime_error(string("Failed to convert sockaddr to string: ") + linux_error());
...
linux_listen_socket::linux_listen_socket(string port)
{
	_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);

	socklen_t cli_len;
	if (_socket < 0)
		throw runtime_error(string("Failed to create linux socket: ") + linux_error());
	...
	if (bind(_socket, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0)
		throw runtime_error(string("Failed to bind linux socket: ") + linux_error());
}

void linux_listen_socket::listen()
{
	if (::listen(_socket, 5) < 0)
		throw runtime_error(string("Error when listening: ") + linux_error());
}
...
bool linux_listen_socket::has_connection()
{
	...
	if (n < 0)
		throw runtime_error(string("Failed to poll linux socket readability: ") + linux_error());
...
linux_data_socket::linux_data_socket(string peer_address, string peer_port)
{
	...
	if (connect(_socket, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0)
		throw runtime_error(string("Socket failed to connect with: ") + linux_error());
}

vector<char> linux_data_socket::receive_data()
{
	vector<char> recv_data{ 500, '0', std::allocator<char>() };
	int n = read(_socket, recv_data.data(), 500);
	if (n < -1)
	{
		cerr << "Failed to read data from linux socket: " << linux_error() << endl;
		return vector<char>();
	}
	recv_data.resize(n);
	return recv_data;
...
bool linux_data_socket::send_data(const vector<char>& data)
{
	int n = write(_socket, data.data(), data.size());
	if (n < 0)
	{
		cerr << "Error sending data: " << linux_error() << endl;
		return false;
	}
...
linux_reuse_nonblock_listen_socket::linux_reuse_nonblock_listen_socket(string port)
{
	...
	if (_socket < 0)
		throw runtime_error(string("Failed to create reusable nonblocking listen socket: ") + linux_error());
	...
	if (n < 0)
	{
		...
		close(_socket);
		throw runtime_error(string("Failed to set socket SO_REUSEADDR: ") + err);
	}
	...
	if (n < 0)
	{
		auto err = linux_error();
		close(_socket);
		throw runtime_error(string("Failed to bind reuseable nonblocking socket: ") + err);
	}
}
...
void linux_reuse_nonblock_listen_socket::listen()
{
	...
	if (n < 1 && n != EINPROGRESS)
		throw runtime_error(string("Failed to listen with: ") + linux_error());
}
...
bool linux_reuse_nonblock_listen_socket::has_connection()
{
	...
	int n = select(1, &poll_read_set, 0, 0, &timeout);
	if (n < 0)
		throw runtime_error(string("Failed to poll linux socket readability (has connection): ") + linux_error());
...
linux_reuse_nonblock_connection_socket::linux_reuse_nonblock_connection_socket(name_data data)
{
	...
	if (_socket < 0)
	{
		auto err = linux_error();
		close(_socket);
		throw runtime_error(string("Failed to create nonblocking socket: ") + err);
	}
	...
	if (n < 0)
	{
		auto err = linux_error();
		close(_socket);
		throw runtime_error(string("Failed to set socket SO_REUSEADDR with: ") + err);
	}
	...
	if (n < 0)
	{
		auto err = linux_error();
		close(_socket);
		throw runtime_error(string("Failed to bind connect socket with: ") + err);
	}
}
...
ConnectionStatus linux_reuse_nonblock_connection_socket::has_connected()
{
	...
	if (n < 1)
	{
		...
		if (n < 0)
			throw runtime_error(string("Failed to select socket error status with: ") + linux_error());
		if (n < 1)
		...
		if (getsockopt(_socket, SOL_SOCKET, SO_ERROR, (char*)&sock_error, &sock_error_size) < 0)
			throw runtime_error(string("Failed to get socket error code with: ") + linux_error());
...
unique_ptr<IDataSocket> linux_reuse_nonblock_connection_socket::convert_to_datasocket()
{
	...
	if (flags < 0)
		throw runtime_error(string("Failed to query socket's flags: ") + linux_error());
	int n = fcntl(_socket, F_SETFL, flags & (~O_NONBLOCK));
	if (n < 0)
		throw runtime_error(string("Failed to set socket as blocking again: ") + linux_error());

	unique_ptr<IDataSocket> out = make_unique<linux_data_socket>(_socket);
	_socket = -1;
	return out;
}
